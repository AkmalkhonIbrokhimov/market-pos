...
create or replace function public.v2_lock_operation_scope(organization_id uuid,device_id uuid,local_operation_id uuid)
returns void language plpgsql set search_path=''as $$
begin
 if organization_id is null or device_id is null or local_operation_id is null then raise exception using errcode='P0001',message='V2_OPERATION_SCOPE_REQUIRED';end if;
 perform pg_advisory_xact_lock(hashtextextended('market-pos-operation:'||organization_id::text||':'||device_id::text||':'||local_operation_id::text,0));
end$$;
...
declare a uuid:=public.v2_current_membership_id(o);i uuid:=gen_random_uuid();c uuid;mv uuid;p jsonb:=jsonb_build_object('organization_id',o,'branch_id',b,'register_id',r,'device_id',d,'opening_cash_amount',opening,'currency_code',cur,'business_date',bd);begin
 ...
 perform public.v2_lock_operation_scope(o,d,op);c:=public.v2_begin_inventory_command(o,b,d,op,'shift.open',p);if(select status='succeeded'from public.command_log where id=c)then return(select entity_id from public.command_log where id=c);end if;
 perform public.v2_lock_register_shift_scope(o,b,r);
...
declare o public.cash_movements%rowtype;s public.shifts_v2%rowtype;a uuid;c uuid;i uuid:=gen_random_uuid();p jsonb;begin
 ...
 perform public.v2_lock_operation_scope(o.organization_id,device_id,local_operation_id);c:=public.v2_use_approved_command(approval_id,o.organization_id,'cash.move.override',local_operation_id,p);if(select status='succeeded'from public.command_log where id=c)then return(select entity_id from public.command_log where id=c);end if;
...
declare s public.shifts_v2%rowtype;cp public.counterparties%rowtype;a uuid;c uuid;i uuid:=gen_random_uuid();entry_id uuid:=gen_random_uuid();p jsonb:=jsonb_build_object('organization_id',organization_id,'branch_id',branch_id,'register_id',register_id,'current_shift_id',current_shift_id,'counterparty_id',counterparty_id,'document_number',document_number,'business_date',business_date,'currency_code',currency_code,'device_id',device_id,'client_created_at',client_created_at,'payments',payments);item jsonb;total numeric:=0;pay_id uuid;balance numeric;begin
 ...
 perform public.v2_lock_operation_scope(organization_id,device_id,local_operation_id);c:=public.v2_begin_inventory_command(organization_id,branch_id,device_id,local_operation_id,'supplier_payment.record',p);if(select status='succeeded'from public.command_log where id=c)then return(select entity_id from public.command_log where id=c);end if;
...
declare o public.supplier_payments%rowtype;s public.shifts_v2%rowtype;a uuid;c uuid;i uuid:=gen_random_uuid();entry_id uuid:=gen_random_uuid();primary_entry public.settlement_entries%rowtype;p jsonb;pay public.payments_v2%rowtype;np uuid;begin
 ...
 perform public.v2_lock_operation_scope(o.organization_id,device_id,local_operation_id);c:=public.v2_use_approved_command(approval_id,o.organization_id,'settlements.reverse',local_operation_id,p);if(select status='succeeded'from public.command_log where id=c)then return(select entity_id from public.command_log where id=c);end if;
 perform public.v2_lock_register_shift_scope(o.organization_id,o.branch_id,o.register_id);select * into s from public.shifts_v2 where id=current_shift_id for update;a:=public.v2_current_membership_id(o.organization_id);perform public.v2_require_open_sales_shift(o.organization_id,o.branch_id,o.register_id,current_shift_id,device_id,a);
...
declare s public.shifts_v2%rowtype;a uuid;c uuid;p jsonb;item jsonb;expected_cash numeric;actual_cash numeric;manual_delta numeric;cash_actual_equiv numeric;cash_expected numeric;card_expected numeric;transfer_expected numeric;cash_diff numeric;card_diff numeric;transfer_diff numeric;line_count integer:=0;cfg jsonb;begin
 ...
 perform public.v2_lock_operation_scope(s.organization_id,d,op);if approval is null then c:=public.v2_begin_inventory_command(s.organization_id,s.branch_id,d,op,'shift.close',p);else c:=public.v2_use_approved_command(approval,s.organization_id,'cash.move.override',op,p);end if;
 if(select status='succeeded'from public.command_log where id=c)then return(select entity_id from public.command_log where id=c);end if;
...
declare i uuid;c uuid;pmt record;begin
 ...
 perform public.v2_lock_operation_scope(o,d,op);perform public.v2_lock_register_shift_scope(o,b,r);if customer is not null then perform public.v2_lock_settlement_scope(o,customer,cur);end if;
...
declare i uuid;c uuid;pmt record;begin perform public.v2_lock_operation_scope(organization_id,device_id,local_operation_id);perform public.v2_lock_register_shift_scope(organization_id,branch_id,register_id);perform public.v2_lock_settlement_scope(organization_id,counterparty_id,currency_code);i:=public.v2_record_debt_payment_0015_cash_base(organization_id,branch_id,register_id,current_shift_id,counterparty_id,document_number,business_date,currency_code,device_id,local_operation_id,client_created_at,payments,allocations);c:=nullif(current_setting('market_pos.current_command_id',true),'')::uuid;for pmt in select id from public.payments_v2 where debt_payment_id=i order by id loop perform public.v2_append_cash_movement_for_payment(pmt.id,c);end loop;return i;end$$;
...
declare i uuid;c uuid;dp public.debt_payments_v2%rowtype;pmt record;begin
 select * into dp from public.debt_payments_v2 where id=debt_payment_id;if not found then raise exception using errcode='P0001',message='V2_DEBT_PAYMENT_NOT_FOUND';end if;perform public.v2_lock_operation_scope(dp.organization_id,device_id,local_operation_id);perform public.v2_lock_register_shift_scope(dp.organization_id,dp.branch_id,dp.register_id);perform public.v2_lock_settlement_scope(dp.organization_id,dp.counterparty_id,dp.currency_code);
