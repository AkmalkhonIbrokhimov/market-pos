...
create table public.purchase_documents(
 ...
 constraint purchase_documents_reversal_shape_check check(reversal_of_id is null or status='posted'),
...
create table public.purchase_additional_costs(
 ...
 constraint purchase_cost_currency_check check(currency_code ~ '^[A-Z]{3}$'),constraint purchase_cost_method_check check(allocation_method in('amount','quantity','weight','manual'))
);
...
create or replace function public.v2_can_view_purchase(o uuid,b uuid,cost boolean default false) returns boolean language sql stable security definer set search_path='' as $$
 ...
 or public.v2_has_support_grant(o,case when cost then 'purchases.cost.view' else 'purchases.view' end)
$$;
...
returns uuid language plpgsql security definer set search_path='' as $$
declare o uuid;b uuid;a uuid;x uuid:=coalesce(i,gen_random_uuid());begin select organization_id,branch_id into o,b from public.purchase_documents where id=doc;a:=public.v2_current_membership_id(o);if a is null or not public.v2_has_permission(o,'purchases.draft.manage',b)then raise exception using errcode='P0001',message='V2_PURCHASE_DRAFT_MANAGE_REQUIRED';end if;perform set_config('market_pos.purchase_command','on',true);
 insert into public.purchase_lines(id,organization_id,purchase_document_id,line_number,product_id,unit_id,quantity,unit_factor,base_quantity,unit_purchase_price,line_amount,expiration_date,supplier_batch_number)
 values(x,o,doc,ln,p,u,q,f,q*f,price,round(q*price,4),exp,sbn) on conflict(id)do update set line_number=excluded.line_number,product_id=excluded.product_id,unit_id=excluded.unit_id,quantity=excluded.quantity,unit_factor=excluded.unit_factor,base_quantity=excluded.base_quantity,unit_purchase_price=excluded.unit_purchase_price,line_amount=excluded.line_amount,expiration_date=excluded.expiration_date,supplier_batch_number=excluded.supplier_batch_number;perform set_config('market_pos.purchase_command','off',true);return x;end$$;
create or replace function public.v2_upsert_purchase_cost(i uuid,doc uuid,ct text,amt numeric,cur char(3),m text)
returns uuid language plpgsql security definer set search_path='' as $$
declare o uuid;b uuid;a uuid;x uuid:=coalesce(i,gen_random_uuid());begin select organization_id,branch_id into o,b from public.purchase_documents where id=doc;a:=public.v2_current_membership_id(o);if a is null or not public.v2_has_permission(o,'purchases.draft.manage',b)then raise exception using errcode='P0001',message='V2_PURCHASE_DRAFT_MANAGE_REQUIRED';end if;perform set_config('market_pos.purchase_command','on',true);insert into public.purchase_additional_costs(id,organization_id,purchase_document_id,cost_type,amount,currency_code,allocation_method)values(x,o,doc,ct,amt,cur,m)on conflict(id)do update set cost_type=excluded.cost_type,amount=excluded.amount,currency_code=excluded.currency_code,allocation_method=excluded.allocation_method;perform set_config('market_pos.purchase_command','off',true);return x;end$$;
...
declare x uuid:=coalesce(i,gen_random_uuid());begin if not public.v2_has_permission(o,'purchases.draft.manage',b)then raise exception using errcode='P0001',message='V2_PURCHASE_DRAFT_MANAGE_REQUIRED';end if;perform set_config('market_pos.purchase_command','on',true);
 if i is null then insert into public.daily_delivery_templates(id,organization_id,branch_id,counterparty_id,name,default_lines)values(x,o,b,cp,n,l);elsif archive then update public.daily_delivery_templates set status='archived',archived_at=clock_timestamp()where id=i;else update public.daily_delivery_templates set name=n,default_lines=l where id=i;end if;perform set_config('market_pos.purchase_command','off',true);return x;end$$;

-- Posting commands ------------------------------------------------------------
create or replace function public.v2_post_purchase(i uuid,op uuid,payload jsonb default '{}'::jsonb)
returns uuid language plpgsql security definer set search_path='' as $$
declare d public.purchase_documents%rowtype;a uuid;c uuid;l public.purchase_lines%rowtype;bt uuid;sub numeric;cost numeric;alloc numeric;begin
 ...
 a:=public.v2_current_membership_id(d.organization_id);c:=public.v2_begin_inventory_command(d.organization_id,d.branch_id,d.device_id,op,'purchase.post',payload||jsonb_build_object('purchase_id',i));if(select status='succeeded'from public.command_log where id=c)then return(select entity_id from public.command_log where id=c);end if;
 ...
 if exists(select 1 from public.purchase_additional_costs pc where pc.purchase_document_id=i and pc.amount<>(select coalesce(sum(a.allocated_amount),0)from public.purchase_cost_allocations a where a.purchase_additional_cost_id=pc.id))then raise exception using errcode='P0001',message='V2_PURCHASE_ALLOCATION_TOTAL_MISMATCH';end if;
 perform set_config('market_pos.purchase_command','on',true);perform set_config('market_pos.inventory_command','on',true);update public.purchase_documents set subtotal_amount=sub,additional_cost_amount=cost,total_amount=sub+cost,status='posted',posted_at=clock_timestamp(),posted_by=a where id=i;
 ...
 end loop;perform public.v2_emit_domain_event(d.organization_id,d.branch_id,d.device_id,a,c,null,'purchase',i,'purchase.posted','PurchasePosted',jsonb_build_object('purchase_id',i));perform public.v2_complete_inventory_command(c,'purchase',i,jsonb_build_object('id',i,'status','posted'));perform set_config('market_pos.purchase_command','off',true);perform set_config('market_pos.inventory_command','off',true);return i;end$$;

create or replace function public.v2_post_daily_delivery(t uuid,n text,bd date,cur char(3),lines jsonb,op uuid)
returns uuid language plpgsql security definer set search_path='' as $$
declare x public.daily_delivery_templates%rowtype;d uuid;l jsonb;seq integer;begin select * into x from public.daily_delivery_templates where id=t and status='active';if not found or not public.v2_has_permission(x.organization_id,'purchases.post_daily',x.branch_id)then raise exception using errcode='P0001',message='V2_PURCHASE_POST_DAILY_REQUIRED';end if;
 select coalesce(max(sequence_number),0)+1 into seq from public.daily_delivery_documents where template_id=t and delivery_date=bd;
 perform set_config('market_pos.purchase_command','on',true);insert into public.purchase_documents(organization_id,branch_id,warehouse_id,counterparty_id,document_number,business_date,currency_code,local_operation_id)
 select x.organization_id,x.branch_id,w.id,x.counterparty_id,n,bd,cur,op from public.warehouses w where w.branch_id=x.branch_id and w.is_primary and w.archived_at is null returning id into d;
 for l in select * from jsonb_array_elements(lines)loop perform public.v2_upsert_purchase_line(null,d,(l->>'line_number')::integer,(l->>'product_id')::uuid,(l->>'unit_id')::uuid,(l->>'quantity')::numeric,(l->>'unit_factor')::numeric,(l->>'unit_purchase_price')::numeric,null,l->>'supplier_batch_number');end loop;
 perform set_config('market_pos.daily_purchase_command','on',true);perform public.v2_post_purchase(d,op,lines);perform set_config('market_pos.purchase_command','on',true);insert into public.daily_delivery_documents(organization_id,template_id,purchase_document_id,delivery_date,sequence_number)values(x.organization_id,t,d,bd,seq);perform set_config('market_pos.purchase_command','off',true);perform set_config('market_pos.daily_purchase_command','off',true);return d;end$$;
...
declare o uuid;b uuid;x uuid:=coalesce(i,gen_random_uuid());begin select organization_id,branch_id into o,b from public.inventory_documents where id=doc;if not public.v2_has_permission(o,'inventory.adjust',b)then raise exception using errcode='P0001',message='V2_INVENTORY_ADJUST_REQUIRED';end if;perform set_config('market_pos.inventory_command','on',true);insert into public.inventory_document_lines(id,organization_id,inventory_document_id,line_number,product_id,batch_id,unit_id,quantity,unit_factor,base_quantity_delta,comment)values(x,o,doc,ln,p,bt,u,q,f,delta,cm)on conflict(id)do update set line_number=excluded.line_number,product_id=excluded.product_id,batch_id=excluded.batch_id,unit_id=excluded.unit_id,quantity=excluded.quantity,unit_factor=excluded.unit_factor,base_quantity_delta=excluded.base_quantity_delta,comment=excluded.comment;perform set_config('market_pos.inventory_command','off',true);return x;end$$;
...
declare d public.inventory_documents%rowtype;l public.inventory_document_lines%rowtype;a uuid;c uuid;needs boolean;begin select * into d from public.inventory_documents where id=i for update;if not found or d.document_type not in('opening','adjustment','write_off')then raise exception using errcode='P0001',message='V2_INVENTORY_DOCUMENT_NOT_POSTABLE';end if;if not public.v2_has_permission(d.organization_id,'inventory.adjust',d.branch_id)then raise exception using errcode='P0001',message='V2_INVENTORY_ADJUST_REQUIRED';end if;a:=public.v2_current_membership_id(d.organization_id);
 ...
 perform set_config('market_pos.inventory_command','on',true);update public.inventory_documents set status='posted',posted_by=a,posted_at=clock_timestamp()where id=i;
...
declare o uuid;b uuid;sw uuid;dw uuid;x uuid:=coalesce(i,gen_random_uuid());begin select organization_id,branch_id,source_warehouse_id,destination_warehouse_id into o,b,sw,dw from public.warehouse_transfers where id=t;if not public.v2_has_permission(o,'inventory.transfer',b)or not public.v2_can_access_warehouse(o,sw)or not public.v2_can_access_warehouse(o,dw)then raise exception using errcode='P0001',message='V2_TRANSFER_BOTH_WAREHOUSES_REQUIRED';end if;perform set_config('market_pos.inventory_command','on',true);insert into public.warehouse_transfer_lines(id,organization_id,warehouse_transfer_id,line_number,product_id,batch_id,unit_id,quantity,unit_factor,base_quantity)values(x,o,t,ln,p,bt,u,q,f,q*f)on conflict(id)do update set line_number=excluded.line_number,product_id=excluded.product_id,batch_id=excluded.batch_id,unit_id=excluded.unit_id,quantity=excluded.quantity,unit_factor=excluded.unit_factor,base_quantity=excluded.base_quantity;perform set_config('market_pos.inventory_command','off',true);return x;end$$;
...
declare d public.purchase_documents%rowtype;r uuid:=gen_random_uuid();a uuid;c uuid;l public.purchase_lines%rowtype;nl uuid;bt public.product_batches_v2%rowtype;om public.inventory_movements%rowtype;begin select * into d from public.purchase_documents where id=i and status='posted' for update;if not found then raise exception using errcode='P0001',message='V2_PURCHASE_NOT_REVERSIBLE';end if;if not public.v2_has_permission(d.organization_id,'purchases.reverse',d.branch_id)then raise exception using errcode='P0001',message='V2_PURCHASE_REVERSE_REQUIRED';end if;if exists(select 1 from public.purchase_documents where reversal_of_id=i)then raise exception using errcode='P0001',message='V2_PURCHASE_ALREADY_REVERSED';end if;
 ...
 insert into public.purchase_documents(id,organization_id,branch_id,warehouse_id,counterparty_id,document_number,business_date,status,currency_code,subtotal_amount,additional_cost_amount,total_amount,device_id,local_operation_id,posted_at,posted_by,reversal_of_id)
 values(r,d.organization_id,d.branch_id,d.warehouse_id,d.counterparty_id,n,current_date,'posted',d.currency_code,d.subtotal_amount,d.additional_cost_amount,d.total_amount,d.device_id,op,clock_timestamp(),a,i);
 for l in select * from public.purchase_lines where purchase_document_id=i order by line_number loop nl:=gen_random_uuid();insert into public.purchase_lines(id,organization_id,purchase_document_id,line_number,product_id,unit_id,quantity,unit_factor,base_quantity,unit_purchase_price,line_amount,expiration_date,supplier_batch_number)values(nl,l.organization_id,r,l.line_number,l.product_id,l.unit_id,l.quantity,l.unit_factor,l.base_quantity,l.unit_purchase_price,l.line_amount,l.expiration_date,l.supplier_batch_number);
