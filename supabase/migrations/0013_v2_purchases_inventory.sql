...
declare bo uuid;wo uuid;wb uuid;co uuid;do_ uuid;db uuid;mo uuid;orig public.purchase_documents%rowtype;
begin
 ...
 if new.reversal_of_id is null then if(tg_op='INSERT'or old.status='draft')and new.counterparty_id is not null then select organization_id into co from public.counterparties where id=new.counterparty_id;if co is distinct from new.organization_id or not exists(select 1 from public.counterparty_roles where counterparty_id=new.counterparty_id and organization_id=new.organization_id and role_code='supplier' and ended_at is null) then raise exception using errcode='P0001',message='V2_PURCHASE_ACTIVE_SUPPLIER_REQUIRED';end if;end if;
 else select * into orig from public.purchase_documents where id=new.reversal_of_id;if not found or orig.organization_id<>new.organization_id or orig.branch_id<>new.branch_id or orig.warehouse_id<>new.warehouse_id or orig.counterparty_id is distinct from new.counterparty_id or orig.currency_code<>new.currency_code then raise exception using errcode='P0001',message='V2_PURCHASE_REVERSAL_SCOPE_MISMATCH';end if;if coalesce(current_setting('market_pos.purchase_reversal_command',true),'')<>'on'then raise exception using errcode='P0001',message='V2_PURCHASE_REVERSAL_COMMAND_CONTEXT_REQUIRED';end if;end if;
...
create policy purchase_documents_select on public.purchase_documents for select to authenticated using(public.v2_can_view_purchase(organization_id,branch_id,true));
...
create or replace function public.v2_can_view_purchase_metadata(i uuid)returns boolean language sql stable security definer set search_path='' as $$select exists(select 1 from public.purchase_documents d where d.id=i and public.v2_can_view_purchase(d.organization_id,d.branch_id,false))$$;
revoke execute on function public.v2_can_view_purchase_metadata(uuid)from public,anon;grant execute on function public.v2_can_view_purchase_metadata(uuid)to authenticated;
create policy daily_documents_select on public.daily_delivery_documents for select to authenticated using(public.v2_can_view_purchase_metadata(purchase_document_id));
...
declare c public.command_log%rowtype;a uuid;h text:=encode(extensions.digest(p::text,'sha256'),'hex');begin
 ...
 if found then if c.command_type<>ct then raise exception using errcode='P0001',message='V2_IDEMPOTENCY_COMMAND_TYPE_MISMATCH';end if;if c.payload_hash<>h then raise exception using errcode='P0001',message='V2_IDEMPOTENCY_PAYLOAD_MISMATCH';end if;return c.id;end if;
...
declare c uuid;h text:=encode(extensions.digest(p::text,'sha256'),'hex');begin select a.command_id into c from public.approval_requests a join public.command_log l on l.id=a.command_id where a.id=aid and a.organization_id=o and a.permission_code=pc and a.status='approved' and a.payload_hash=h and l.organization_id=o and l.local_operation_id=op and l.command_type=pc and l.payload_hash=h and l.status in('processing','succeeded')and(l.status='succeeded'or a.expires_at>now());if c is null then raise exception using errcode='P0001',message='V2_APPROVED_REQUEST_REQUIRED';end if;return c;end$$;
...
returns uuid language plpgsql security definer set search_path='' as $$
declare a uuid:=public.v2_current_membership_id(o);i uuid:=gen_random_uuid();c uuid;p jsonb:=jsonb_build_object('organization_id',o,'branch_id',b,'warehouse_id',w,'counterparty_id',cp,'document_number',n,'business_date',bd,'currency_code',cur,'device_id',d);begin
...
declare d public.purchase_documents%rowtype;r uuid:=gen_random_uuid();a uuid;c uuid;l public.purchase_lines%rowtype;pc public.purchase_additional_costs%rowtype;al public.purchase_cost_allocations%rowtype;nl uuid;rc uuid;target_line uuid;bt public.product_batches_v2%rowtype;om public.inventory_movements%rowtype;begin select * into d from public.purchase_documents where id=i for update;if not found then raise exception using errcode='P0001',message='V2_PURCHASE_NOT_REVERSIBLE';end if;if not public.v2_has_permission(d.organization_id,'purchases.reverse',d.branch_id)then raise exception using errcode='P0001',message='V2_PURCHASE_REVERSE_REQUIRED';end if;a:=public.v2_current_membership_id(d.organization_id);c:=public.v2_use_approved_command(approval,d.organization_id,'purchases.reverse',op,payload||jsonb_build_object('purchase_id',i,'document_number',n));if(select status='succeeded'from public.command_log where id=c)then return(select entity_id from public.command_log where id=c);end if;if exists(select 1 from public.purchase_documents where reversal_of_id=i)then raise exception using errcode='P0001',message='V2_PURCHASE_ALREADY_REVERSED';end if;if d.status<>'posted'then raise exception using errcode='P0001',message='V2_PURCHASE_NOT_REVERSIBLE';end if;
 ...
 update public.purchase_documents set status='reversed'where id=i;perform public.v2_emit_domain_event(d.organization_id,d.branch_id,d.device_id,a,c,approval,'purchase',r,'purchase.reversed','PurchaseReversed',jsonb_build_object('original_id',i,'reversal_id',r));perform public.v2_complete_inventory_command(c,'purchase',r,jsonb_build_object('id',r,'status','posted','reversal_of',i));perform set_config('market_pos.purchase_reversal_command','off',true);perform set_config('market_pos.purchase_command','off',true);perform set_config('market_pos.inventory_command','off',true);return r;end$$;
...
declare t public.warehouse_transfers%rowtype;r uuid:=gen_random_uuid();a uuid;c uuid;l public.warehouse_transfer_lines%rowtype;nl uuid;oo public.inventory_movements%rowtype;ii public.inventory_movements%rowtype;begin select * into t from public.warehouse_transfers where id=i for update;if not found then raise exception using errcode='P0001',message='V2_TRANSFER_NOT_REVERSIBLE';end if;if not public.v2_has_permission(t.organization_id,'inventory.transfer',t.branch_id)or not public.v2_can_access_warehouse(t.organization_id,t.source_warehouse_id)or not public.v2_can_access_warehouse(t.organization_id,t.destination_warehouse_id)then raise exception using errcode='P0001',message='V2_TRANSFER_BOTH_WAREHOUSES_REQUIRED';end if;a:=public.v2_current_membership_id(t.organization_id);c:=public.v2_begin_inventory_command(t.organization_id,t.branch_id,t.device_id,op,'inventory.transfer.reverse',payload||jsonb_build_object('transfer_id',i,'document_number',n));if(select status='succeeded'from public.command_log where id=c)then return(select entity_id from public.command_log where id=c);end if;if exists(select 1 from public.warehouse_transfers where reversal_of_id=i)then raise exception using errcode='P0001',message='V2_TRANSFER_ALREADY_REVERSED';end if;if t.status<>'posted'then raise exception using errcode='P0001',message='V2_TRANSFER_NOT_REVERSIBLE';end if;perform set_config('market_pos.inventory_command','on',true);
...
declare d public.inventory_documents%rowtype;r uuid:=gen_random_uuid();a uuid;c uuid;l public.inventory_document_lines%rowtype;nl uuid;om public.inventory_movements%rowtype;begin select * into d from public.inventory_documents where id=i for update;if not found then raise exception using errcode='P0001',message='V2_INVENTORY_DOCUMENT_NOT_REVERSIBLE';end if;if not public.v2_has_permission(d.organization_id,'inventory.adjust',d.branch_id)then raise exception using errcode='P0001',message='V2_INVENTORY_ADJUST_REQUIRED';end if;a:=public.v2_current_membership_id(d.organization_id);c:=public.v2_use_approved_command(approval,d.organization_id,'inventory.adjust',op,payload||jsonb_build_object('document_id',i,'document_number',n));if(select status='succeeded'from public.command_log where id=c)then return(select entity_id from public.command_log where id=c);end if;if exists(select 1 from public.inventory_documents where reversal_of_id=i)then raise exception using errcode='P0001',message='V2_INVENTORY_DOCUMENT_ALREADY_REVERSED';end if;if d.status<>'posted'then raise exception using errcode='P0001',message='V2_INVENTORY_DOCUMENT_NOT_REVERSIBLE';end if;perform set_config('market_pos.inventory_command','on',true);
