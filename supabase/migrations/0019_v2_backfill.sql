...
begin
  ...
  insert into public.migration_backfill_runs(organization_id,mode,source_snapshot_at)
  values(organization_id,mode,clock_timestamp())returning id into run_id;
...
begin
  ...
  if phase='identity_profiles'then
    ...
    for row_data in
      ...
      where s.organization_id=run.organization_id order by legacy_order loop
        ...
        if row_data.kind='store'then
          if run.mode='apply'then
            ...
            for source_org in select usa.user_id from public.user_store_access usa
              ...
              where usa.store_id=row_data.id and u.role::text='seller'loop
              select m.target_id into membership_target
              from public.migration_entity_mappings m
              where m.organization_id=run.organization_id and m.legacy_table='users'
          ...
          elsif run.mode='apply'and exists(select 1 from public.devices_v2 d
        ...
        if run.mode='apply'then
          ...
          if target is null then
            ...
            if row_data.kind='brands'then
              ...
              generated_code:='MIG-U-'||case when row_data.id is null then
              ...
              generated_code:=coalesce(nullif(btrim(row_data.code),''),
                'MIG-T-'||public.v2_backfill_uuid_fragment(row_data.id));
        ...
        if run.mode='apply'then
          ...
          if generated_code is not null and exists(select 1 from public.products_v2 p
            where p.organization_id=run.organization_id
              and lower(p.sku)=lower(generated_code)
              and p.legacy_product_id is distinct from row_data.id)then
            perform public.v2_backfill_finding(run.id,phase,'products',row_data.id::text,
          ...
          if target is null then
            ...
            where m.organization_id=run.organization_id and m.legacy_table='product_types'
              and m.legacy_key=row_data.product_type_id::text
              and m.target_table='product_types_v2';
          ...
          if nullif(btrim(row_data.barcode),'')is not null then
            normalized:=public.v2_normalize_barcode(row_data.barcode);
            if exists(select 1 from public.product_barcodes b
              where b.organization_id=run.organization_id
                and b.normalized_barcode=normalized and b.product_id<>target
                and b.archived_at is null)then
              perform public.v2_backfill_finding(run.id,phase,'products',row_data.id::text,
      ...
      begin
        ...
        if run.mode='apply'then
          ...
          if target_amount is null then
            ...
            perform set_config('market_pos.pricing_command','off',true);
          elsif target_amount<>row_data.sale_price then
            perform public.v2_backfill_finding(run.id,phase,'products',row_data.id::text,
